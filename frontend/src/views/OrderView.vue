    ...
    <div class="page-container">
        ...
        <div class="main-container">
            <div class="order-container">
                ...
                <div
                    class="item-order-searchbar"
                    v-for="order in filteredOrders"
                    :key="order.id"
                ></div>

                <div
                    class="item-error-searchbar"
                    v-if="searchQuery && !filteredOrders.length"
                >
...
export default {
    ...
    methods: {
        ...
        async fetchOrdersFilter() {
            let endpoint = `${apiBaseUrl}/orders/${this.selectedStatus}-orders`
            if (this.selectedStatus === 'all') {
                endpoint = `${apiBaseUrl}/orders/all-orders`
            }
        ...
        async fetchWorkerOrders(workerId) {
            try {
                const response = await axios.get(
                    `${apiBaseUrl}/orders/worker/${workerId}`
                )
                this.orders = response.data.sort(
                    (a, b) => new Date(b.date) - new Date(a.date)
                )
