    ...
    <div class="page-container">
        ...
        <div class="main-container">
            <div class="order-detail-container" v-if="order">
                ...
                <button
                    class="edit-button"
                    v-if="userRole === 'USER' && order.status === 'UNPAID'"
                    @click="editOrder"
                >
                    Edit
                </button>
                ...
                <p class="order-id-text">
                    Order ID: {{ this.$route.params.orderId }}
                </p>
                ...
                <div class="product-list-box">
                    ...
                    <table class="product-list-table">
                        ...
                        <tbody>
                            <tr
                                v-for="product in order.productDetails"
                                :key="product.id"
                            >
                                <td>{{ product.id }}</td>
    ...
    methods: {
        async fetchOrderDetails() {
            try {
                const response = await axios.get(
                    `${apiBaseUrl}/orders/order-detail/${this.$route.params.orderId}`
                )
        ...
        deleteOrder() {
            ...
            if (confirm('Do you want to delete this order?')) {
                // ยืนยันการลบ
                axios
                    .delete(`${apiBaseUrl}/orders/${orderId}/delete`)
                    .then((response) => {
                    ...
                    .catch((error) => {
                        ...
                        if (error.response && error.response.status === 403) {
                            alert(
                                'Order cannot be deleted due to invalid orders status'
                            )
                        } else {
        ...
        checked() {
            ...
            fetch(
                `${apiBaseUrl}/orders/order-detail/${orderId}/change-status?staus=CHECKED`,
                {
                    method: 'POST',
                    headers: {
        ...
        clickComplete() {
            ...
            fetch(
                `${apiBaseUrl}/orders/order-detail/${orderId}/change-status`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
        ...
        showFileModal() {
            this.fileUrl = `${apiBaseUrl}/orders/order-detail/${this.$route.params.orderId}/shipment-doc`
