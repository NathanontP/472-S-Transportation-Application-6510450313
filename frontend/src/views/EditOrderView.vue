            ...
            <form @submit.prevent="submitOrder" class="form">
                <div class="customer-info">
                    ...
                    <div>
                        ...
                        <select v-model="orderRequest.paymentMethod" required>
                            ...
                            <option value="creditCardService">
                                Credit Card
                            </option>
                        </select>
                    </div>
                ...
                <div class="scroll-container">
                    <div
                        ...
                        v-for="product in orderRequest.productDetails"
                        :key="product.id"
                    >
                        <input
                            type="text"
    ...
    methods: {
        async fetchOrderData(orderId, orderDetails) {
            try {
                const response = await fetch(
                    `${apiBaseUrl}/orders/edit-order/${this.$route.params.orderId}`
                )
                if (response.ok) {
                    ...
                    this.orderRequest.productDetails =
        ...
        async submitOrder() {
            ...
            const url = this.orderId
                ? `${apiBaseUrl}/orders/edit-order/${this.$route.params.orderId}`
                : `${apiBaseUrl}/orders/create-order`
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    alert(
                        `Error: ${
                            errorData.message || 'Failed to process order'
                        }`
                    )
                }
            } catch (error) {
        ...
        cancelEdit() {
            if (this.originalOrderRequest) {
                this.orderRequest = JSON.parse(
                    JSON.stringify(this.originalOrderRequest)
                )
            }
            this.$router.back()
        },
